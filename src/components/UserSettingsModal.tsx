...
const UserSettingsModal: React.FC<UserSettingsModalProps> = ({ isVisible, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50 px-4">
      <div className="bg-white rounded-lg w-full max-w-md mx-auto overflow-hidden" style={{minHeight:"640px", minWidth:"43em"}}>
        ...
        <div className="flex flex-row">
          <div className="border-r border-gray-200">
            <div className={`cursor-pointer p-4 ${activeTab === 'General' ? 'bg-gray-200' : ''}`}
                 onClick={() => setActiveTab('General')}>General
            </div>
            <div className={`cursor-pointer p-4 ${activeTab === 'Instructions' ? 'bg-gray-200' : ''}`}
                 onClick={() => setActiveTab('Instructions')}>Instructions
            </div>
            <div className={`cursor-pointer p-4 ${activeTab === 'Storage' ? 'bg-gray-200' : ''}`}
            ...
            <div className={`${activeTab === 'Instructions' ? '' : 'hidden'}`}>
              <div className="border-b border-token-border-light pb-3 last-of-type:border-b-0">
                <textarea
                  ...
                  className="resize-y rounded overflow-y-auto h-72 w-full max-h-[60vh] md:max-h-[calc(100vh-300px)] shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                ></textarea>
            </div>
            </div>
