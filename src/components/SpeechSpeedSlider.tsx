...
const SpeechSpeedSlider: React.FC<SpeechSpeedSliderProps> = ({ value, onValueChange }) => {
  ...
  return (
    <div>
      <p className='mb-2'>Adjust the speech speed to your preference. Lower values will slow down the speech, while higher values will speed it up.</p>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2px' }}>
        <span className='text-gray-600' style={{ alignSelf: 'flex-start', fontSize: '12px' }}>{t('slower-label')}</span>
        <span className='text-gray-600'  style={{ alignSelf: 'flex-end', fontSize: '12px' }}>{t('faster-label')}</span>
      </div>
      <Slider
